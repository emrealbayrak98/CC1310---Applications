 ...
 * @returns 0 on success, 1 on sensor error, 2 on wrong ID.
...
uint8_t bmp180_init(I2C_Handle* i2c, I2C_Transaction* i2cTransaction,bmp180_t *bmp180)
{
    ...
    bmp180_read(i2c,bmp180, ID, rbuffer, 1);
    if (rbuffer[0] != 0x55) {
        return 2;
    }
 ...
 * @returns uncompensated temperature value.
 * @param hi2cx I2C handle.
 * @param bmp180 `bmp180_t` struct to initialize.
 * */
static int32_t _bmp180_read_ut(I2C_Handle* i2c,bmp180_t *bmp180)
{
    uint8_t write_data[2] = {0,0x2E}, ut_data[2];

    bmp180_write(i2c,bmp180, CTRL_MEAS, write_data, 2);
    usleep(5*1000);
    bmp180_read(i2c,bmp180, OUT_MSB, ut_data, 2);
 ...
 * @returns uncompensated pressure value.
 * @param hi2cx I2C handle.
 * @param bmp180 `bmp180_t` struct to initialize.
 * */
static int32_t _bmp180_read_up(I2C_Handle* i2c,bmp180_t *bmp180)
{
    uint8_t write_data[2] = {0,0x34 + (bmp180->oss << 6)},up_data[3];
    bmp180_write(i2c,bmp180, CTRL_MEAS, write_data, 2);
    uint8_t wait = 0;
    ...
    usleep(wait*1000);
    bmp180_read(i2c,bmp180, OUT_MSB, up_data, 3);
